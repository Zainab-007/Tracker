...
app = Flask(__name__)
app.secret_key = os.getenv('SECRET_KEY', 'smarttracker2024')

# ---- MySQL Config from .env (with safe defaults) ----
app.config['MYSQL_HOST'] = os.getenv('MYSQL_HOST', 'localhost')
app.config['MYSQL_USER'] = os.getenv('MYSQL_USER', 'root')
app.config['MYSQL_PASSWORD'] = os.getenv('MYSQL_PASSWORD', 'zainab13')
app.config['MYSQL_DB'] = os.getenv('MYSQL_DB', 'interntrack')
...
# ---- TF-IDF (unchanged - already excellent) ----
...
# ------------------------------------------------------------------
# GLOBAL ERROR HANDLERS (fixes "No try/except around DB queries")
# ------------------------------------------------------------------
@app.errorhandler(404)
def page_not_found(e):
    user_name = session.get('user_name')
    return render_template('404.html', user_name=user_name) if 'user_name' in session else "404 - Page Not Found", 404
...
# ------------------------------------------------------------------
# ROUTES (only changed parts shown with comments)
# ------------------------------------------------------------------
@app.route('/')
def home():
    ...
    if request.method == 'POST':
        ...
        if len(password) < 6:
            ...
            try:
                cur.execute("""
                    INSERT INTO users 
                    (name, username, email, college, grad_year, password, skills) 
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                """, (name, username, email, college, grad_year, hashed_password, skills))
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            stored_password = user[4]   # password hash column
            
            # Safety check - prevent NoneType error
            if stored_password and check_password_hash(stored_password, password):
                session['user_id']   = user[0]
                session['user_name'] = user[1]
                return redirect('/dashboard')
            else:
                error = "Wrong email/username or password. Try again."
        else:
            error = "Wrong email/username or password. Try again."
...
def skill_gap():
    ...
    # Improved keyword list (expanded + more relevant terms)
    common_skills = [
        'python', 'javascript', 'java', 'sql', 'html', 'css', 'react', 'flask', 'django',
        'machine learning', 'data science', 'ai', 'ml', 'nlp', 'deep learning',
        'excel', 'tableau', 'power bi', 'photoshop', 'figma', 'ui ux', 'marketing',
        'communication', 'web development', 'android', 'ios', 'nodejs', 'php', 'c++',
        'aws', 'azure', 'cloud', 'devops', 'git', 'docker', 'kubernetes'
    ]
    ...
    for i in internships:
        ...
        gap_data.append({
            'title': i[1], 'company': i[2], 'field': i[4],
...
def my_applications():
    ...
    cur.execute("""
        ...
        WHERE a.user_id = %s ORDER BY a.applied_date DESC
    """, [session['user_id']])
    ...
    offered   = sum(1 for a in applications if a[4] == 'Offered')
    rejected  = sum(1 for a in applications if a[4] == 'Rejected')
    interview = sum(1 for a in applications if a[4] == 'Interview')
    in_progress = total - offered - rejected
...
# ---- Admin Dashboard (View all internships) ----
...
def admin_dashboard():
    ...
    internships = cur.fetchall()
    cur.close()
    return render_template('admin_dashboard.html', internships=internships)
...
def admin_add():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "INSERT INTO internships (title, company, description, field, stipend, deadline, apply_link) VALUES (%s,%s,%s,%s,%s,%s,%s)",
            (title, company, description, field, stipend, deadline, apply_link)
        )
        mysql.connection.commit()
        cur.close()
        success = f"✅ '{title}' at {company} added successfully!"
    return render_template('admin_add.html', success=success)
...
def admin_edit(id):
    ...
    if request.method == 'POST':
        ...
        apply_link  = request.form['apply_link']
        cur.execute(
            "UPDATE internships SET title=%s, company=%s, description=%s, field=%s, stipend=%s, deadline=%s, apply_link=%s WHERE id=%s",
            (title, company, description, field, stipend, deadline, apply_link, id)
        )
        mysql.connection.commit()
        cur.close()
        return redirect('/admin/dashboard')
    ...
    internship = cur.fetchone()
    cur.close()
    return render_template('admin_edit.html', internship=internship)
...
def admin_delete(id):
    ...
    cur.execute("DELETE FROM bookmarks WHERE internship_id = %s", [id])
    cur.execute("DELETE FROM reviews WHERE internship_id = %s", [id])
    cur.execute("DELETE FROM recently_viewed WHERE internship_id = %s", [id])
...
def admin_stats():
    ...
    total_applications = cur.fetchone()[0]
    cur.execute("SELECT name, email, skills FROM users ORDER BY id DESC LIMIT 10")
    recent_users = cur.fetchall()
    cur.execute("SELECT i.title, i.company, COUNT(a.id) as apps FROM internships i LEFT JOIN applications a ON i.id = a.internship_id GROUP BY i.id ORDER BY apps DESC LIMIT 5")
